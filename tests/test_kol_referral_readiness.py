...
class KOLReferralReadinessTests(unittest.TestCase):
 def setUp(self):
  ...
  self.authority={"source_class":"FLOP_LABS_OFFICIAL","source_id":"official-kol-source","source_document_sha256":self.source_hash,"program_id":"kol-program","announcement_version":"announcement-v1","rules_version":"rules-v1","rules_sha256":self.rules_hash,"referrals":[{k:self.referral[k] for k in kol.AUTH_REF_FIELDS}]}
 ...
 def test_source_and_referral_binding_mismatches_fail(self):
  for change in ({"source_document_sha256":"0"*64},{"rules_version":"rules-v2"},{"rules_sha256":"0"*64}):self.assertEqual(self.assess([{**self.program,**change}])["errors"],["SOURCE_UNVERIFIED"])
