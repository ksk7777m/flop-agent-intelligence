...
class TestnetReadinessTests(unittest.TestCase):
 ...
 def test_verified_sources_reach_human_review_only(self):
  ...
  for field in ("claim_authorized","inference_spend_authorized","ready_to_act","authorized_to_act","live_action_enabled"):self.assertFalse(result[field])
 ...
 def test_raw_material_extra_fields_and_bool_nonce_are_rejected_without_echo(self):
  attacks=({**self.testnet,"raw_url":"https://secret.invalid/?key=PRIVATE"},{**self.testnet,"observation_nonce":True})
 ...
 def test_schema_semantic_resource_and_reachability_boundaries(self):
  result=self.assess();forged=copy.deepcopy(result);forged["ready_to_act"]=True;forged["artifact_id"]=readiness._hash(readiness._canon({k:v for k,v in forged.items() if k!="artifact_id"}))
