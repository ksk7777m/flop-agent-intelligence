...
durable handoff records intent before key loading and a POST attempt
before the sole transport invocation, then blocks re-signing and re-sending
across restart. Fixed identity, signer, bounded POST, and unsigned GET adapters
...
The independent Engagement Monitor is implemented with collection and scheduling disabled. A future one-shot run reads only the exact public aggregate URL `https://technocore.chat/rooms?format=json&limit=200`, enforces a 2 MiB successful-response limit, a 20-second socket timeout, and one absolute monotonic budget with a 30-second maximum that also bounds DNS resolution. The collector cannot disable this deadline and never retries. An internal worker must establish and prove an isolated session, then wait for the parent's READY acknowledgment before performing fetch, normalization, and schema validation. The parent strictly validates a 256 KiB IPC envelope before committing once under a stable `0600` lock file. Timeout cleanup signals only the verified, unreaped worker group and releases ownership after reaping; bounded safety cleanup may slightly overrun the normal deadline. Uncommitted work is discarded without rollback. History and preview publication use complete private fsynced candidates and atomic replacement. History is `PRE_COMMIT` before replacement, `COMMITTED` when replacement succeeds, and `DURABLE` only after directory fsync; the deadline cannot interrupt between replacement and publication of `COMMITTED`. Result success means history was committed. Durability warning, preview state/warning, and cleanup state/error are independent validated fields, allowing simultaneous post-commit conditions to remain visible; preview `FAILED` means publication was attempted. Final IPC/process cleanup is merged before result revalidation, so cleanup failure cannot obscure a committed history or prior warning. History remains authoritative throughout. The collector discards raw and error bodies and makes no room, KV, mailbox, wallet, or write request. Scheduler V0.1 adds only a disabled local policy wrapper with a 60-minute design cadence, 30-minute hard floor, zero retries, 24-request rolling ceiling, and two-failure human-reset circuit breaker; no scheduler is installed or activated. See [Scheduler V0.1](docs/ENGAGEMENT_SCHEDULER.md). Checked-in APIs honestly report **NO REVIEWED ENGAGEMENT HISTORY YET**.
