...
import hashlib, hmac, json, os, re, secrets, socket, sqlite3, struct, sys, threading, weakref
...
def _build_public_facade(trusted_root:Path,clock:Callable[[],datetime],store_id:str)->tuple[Callable[...,Any],...]:
    sha,mac,digest,jsonm,sql,osm,stm=hashlib.sha256,hmac.new,hmac.compare_digest,json,sqlite3,os,stat_module
    safe,hex64,decimal,context_match=SAFE_TEXT.fullmatch,HEX64.fullmatch,DECIMAL.fullmatch,CONTEXT.fullmatch
    dt,utc=datetime,timezone.utc;action_t,state_t,effect_t=CanonicalAction,ReplayState,EffectClass
    retry_t,error_t,proxy=RetryClassification,ReplaySafetyError,MappingProxyType
    policy,schema=POLICY_VERSION,SCHEMA_VERSION;allowed_actions,target_prefixes,action_schema=ACTION_CLASSES,TARGET_PREFIXES,ACTION_SCHEMA_VERSION
    ...
    alphabet="123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";root=Path(trusted_root).absolute();dirname,filename="replay-safety","ledger.sqlite3";db=root/dirname/filename;mutex=threading.RLock()
    ...
    def valid_did(value:Any)->bool:
        if type(value) is not str or not value.startswith("did:key:z"):return False
        encoded=value[9:]
        ...
        raw=b"\0"*(len(encoded)-len(encoded.lstrip("1")))+body
        if len(raw)!=34 or raw[:2]!=b"\xed\x01":return False
        leading=len(raw)-len(raw.lstrip(b"\0"));number=int.from_bytes(raw,"big");canonical=""
        while number:number,remainder=divmod(number,58);canonical=alphabet[remainder]+canonical
        return encoded=="1"*leading+canonical
    def validate_action(a:CanonicalAction)->None:
        ...
        if not valid_did(a.actor_did):raise error_t("ACTOR_DID_INVALID","actor_did","canonical Ed25519 did:key required")
    ...
    def material(a:CanonicalAction)->bytes:
        validate_action(a);return jsonm.dumps({"actor_did":a.actor_did,"action_class":a.action_class,"context":a.context,"nonce":a.nonce,"signed_payload_sha256":a.signed_payload_sha256,"signing_bytes_sha256":a.signing_bytes_sha256,"target":a.target,"schema_version":a.schema_version,"policy_version":policy},sort_keys=True,separators=(",",":"),ensure_ascii=False).encode()
    def operate(operation:str,a:CanonicalAction)->Any:
        canonical=material(a);rid=sha(b"FLOP-REPLAY-ID\0"+canonical).hexdigest()
        if operation=="replay_id":return rid
        if operation not in {"observe","inspect"}:raise error_t("OPERATION_INVALID","operation","public replay operation required")
        scope_material=jsonm.dumps({"actor_did":a.actor_did,"context":a.context,"nonce":a.nonce,"schema_version":a.schema_version},sort_keys=True,separators=(",",":")).encode();scope=sha(b"FLOP-NONCE-SCOPE\0"+scope_material).hexdigest()
        value=clock()
        ...
        when=value.astimezone(utc).isoformat();dflags=osm.O_RDONLY|getattr(osm,"O_DIRECTORY",0)|getattr(osm,"O_NOFOLLOW",0)
        ...
        rfd=osm.open("/",dflags);cfd=fd=credential_fd=None;con=None
        try:
            for part in root.parts[1:]:following=osm.open(part,dflags,dir_fd=rfd);osm.close(rfd);rfd=following
            ri=osm.fstat(rfd)
            if not stm.S_ISDIR(ri.st_mode) or ri.st_uid!=osm.getuid() or ri.st_mode&0o777!=0o700:raise error_t("STORAGE_ROOT_UNSAFE","storage","owned 0700 real directory required")
            try:osm.mkdir(dirname,0o700,dir_fd=rfd)
            except FileExistsError:pass
            cfd=osm.open(dirname,dflags,dir_fd=rfd);ci=osm.fstat(cfd)
            ...
            try:credential_fd=osm.open("store.credential",osm.O_RDONLY|getattr(osm,"O_NOFOLLOW",0),dir_fd=cfd)
                ...
                credential_fd=osm.open("store.credential",osm.O_RDWR|osm.O_CREAT|osm.O_EXCL|getattr(osm,"O_NOFOLLOW",0),0o600,dir_fd=cfd);credential=osm.urandom(32);osm.write(credential_fd,credential);osm.fsync(credential_fd);osm.fsync(cfd);osm.lseek(credential_fd,0,osm.SEEK_SET)
            ...
            if operation=="observe":
                ...
                if prior and prior[0]!=rid:
                    conflict_id=sha(f"FLOP-CONFLICT|{scope}|{prior[0]}|{rid}".encode()).hexdigest();con.execute("INSERT OR IGNORE INTO replay_conflicts VALUES(?,?,?,?,?,?)",(conflict_id,scope,prior[0],rid,sha(canonical).hexdigest(),when));con.commit();return proxy({"status":"DESCRIPTIVE_ONLY","decision":"REPLAY_IDENTITY_CONFLICT","replay_id":rid,"authoritative_replay_id":prior[0],"authoritative_state":prior[1],"state":state_t.REPLAY_IDENTITY_CONFLICT.value,"retry_classification":retry_t.HUMAN_REVIEW_REQUIRED.value})
                if prior is None:
                    con.execute("INSERT INTO replay_records VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(rid,scope,a.actor_did,a.action_class,a.context,a.nonce,a.signed_payload_sha256,a.signing_bytes_sha256,a.target,a.schema_version,state_t.OBSERVED.value,1,when,when));event_hash=sha(f"{rid}|0|None|OBSERVED|{when}".encode()).hexdigest();con.execute("INSERT INTO replay_events(replay_id,from_state,to_state,occurred_at,event_hash) VALUES(?,?,?,?,?)",(rid,None,state_t.OBSERVED.value,when,event_hash));decision,count,current="FIRST_OBSERVATION",1,state_t.OBSERVED
                ...
                con.commit();return proxy({"status":"DESCRIPTIVE_ONLY","decision":decision,"replay_id":rid,"state":current.value,"observation_count":count,"retry_classification":retries[current].value})
            ...
            effect=con.execute("SELECT effect_class,confirmation_evidence_type,confirmation_evidence_hash FROM effects WHERE replay_id=? ORDER BY length(attempt_number) DESC,attempt_number DESC LIMIT 1",(rid,)).fetchone();current=state_t(row[2]);classification=unknown[effect_t(effect[0])] if current is state_t.EFFECT_OUTCOME_UNKNOWN and effect else retries[current]
            ...
            if credential_fd is not None:osm.close(credential_fd)
            if fd is not None:osm.close(fd)
            if cfd is not None:osm.close(cfd)
            osm.close(rfd)
    def observe(a:CanonicalAction)->Mapping[str,Any]:return operate("observe",a)
    def inspect_action(a:CanonicalAction)->Mapping[str,Any]:return operate("inspect",a)
    def replay_id(a:CanonicalAction)->str:return operate("replay_id",a)
...
def _production_facade()->tuple[Callable[...,Any],...]:
    ...
    return _build_public_facade(_PRODUCTION_ROOT,_production_clock,store_id)
observe_action,inspect_action,canonical_replay_id=_production_facade()
del _build_public_facade
...
def _new_test_family(root:Path,clock:Callable[[],datetime])->SimpleNamespace:
    ...
    def valid_did(value:Any)->bool:
        ...
        for char in encoded:
            ...
            if position<0:return False
            number=number*58+position
        body=number.to_bytes((number.bit_length()+7)//8,"big") if number else b"";raw=b"\0"*(len(encoded)-len(encoded.lstrip("1")))+body
        if len(raw)!=34 or raw[:2]!=b"\xed\x01":return False
        leading=len(raw)-len(raw.lstrip(b"\0"));number=int.from_bytes(raw,"big");canonical=""
        while number:number,remainder=divmod(number,58);canonical=alphabet[remainder]+canonical
        return encoded=="1"*leading+canonical
...
IPC_PROTOCOL_VERSION="replay-ipc-v1"
IPC_MAX_REQUEST_BYTES=16_384
IPC_MAX_RESPONSE_BYTES=32_768
IPC_FIELDS=frozenset({"version","request_id","auth","command","policy_version","action"})
IPC_ACTION_FIELDS=frozenset({"actor_did","action_class","context","nonce","signed_payload_sha256","signing_bytes_sha256","target","schema_version"})
IPC_COMMANDS=MappingProxyType({"OBSERVE":observe_action,"INSPECT":inspect_action,"REPLAY_ID":canonical_replay_id})

def _ipc_recv_exact(channel:socket.socket,size:int)->bytes:
    chunks=[];remaining=size
    while remaining:
        chunk=channel.recv(remaining)
        if not chunk:raise ReplaySafetyError("IPC_TRUNCATED","request","complete framed request required")
        chunks.append(chunk);remaining-=len(chunk)
    return b"".join(chunks)

def _ipc_safe_error(exc:BaseException)->dict[str,str]:
    if isinstance(exc,ReplaySafetyError):
        return {"code":exc.code,"field":str(exc.metadata["field"]),"reason":str(exc.metadata["reason"])}
    if isinstance(exc,(ValueError,TypeError,UnicodeError,json.JSONDecodeError)):
        return {"code":"IPC_REQUEST_INVALID","field":"request","reason":"canonical bounded request required"}
    return {"code":"IPC_OPERATION_FAILED","field":"operation","reason":"helper rejected operation"}

def _serve_ipc_once(fd:int,expected_auth:str)->int:
    channel=socket.socket(fileno=fd);request_id="INVALID"
    try:
        if hasattr(channel,"getpeereid"):
            peer_uid,_peer_gid=channel.getpeereid()
            if peer_uid!=os.getuid():raise ReplaySafetyError("IPC_PEER_REJECTED","peer","same local account required")
        size=struct.unpack("!I",_ipc_recv_exact(channel,4))[0]
        if size==0 or size>IPC_MAX_REQUEST_BYTES:raise ReplaySafetyError("IPC_REQUEST_TOO_LARGE","request","bounded request required")
        value=json.loads(_ipc_recv_exact(channel,size).decode("utf-8"))
        if type(value) is not dict or frozenset(value)!=IPC_FIELDS:raise ReplaySafetyError("IPC_SCHEMA_INVALID","request","exact command fields required")
        request_id=value["request_id"]
        if type(request_id) is not str or re.fullmatch(r"[0-9a-f]{32}",request_id) is None:raise ReplaySafetyError("IPC_REQUEST_ID_INVALID","request_id","canonical correlation ID required")
        supplied=value["auth"]
        if type(supplied) is not str or not hmac.compare_digest(supplied,expected_auth):raise ReplaySafetyError("IPC_AUTHENTICATION_FAILED","auth","authenticated inherited channel required")
        if value["version"]!=IPC_PROTOCOL_VERSION or value["policy_version"]!=POLICY_VERSION:raise ReplaySafetyError("IPC_VERSION_INVALID","version","exact reviewed protocol and policy required")
        command=value["command"]
        if type(command) is not str or command not in IPC_COMMANDS:raise ReplaySafetyError("IPC_COMMAND_UNSUPPORTED","command","reviewed command required")
        raw=value["action"]
        if type(raw) is not dict or frozenset(raw)!=IPC_ACTION_FIELDS:raise ReplaySafetyError("IPC_ACTION_SCHEMA_INVALID","action","exact canonical action fields required")
        action=CanonicalAction(**raw);result=IPC_COMMANDS[command](action)
        if isinstance(result,Mapping):result=dict(result)
        response={"version":IPC_PROTOCOL_VERSION,"request_id":request_id,"ok":True,"result":result,"error":None}
    except BaseException as exc:
        response={"version":IPC_PROTOCOL_VERSION,"request_id":request_id,"ok":False,"result":None,"error":_ipc_safe_error(exc)}
    encoded=json.dumps(response,sort_keys=True,separators=(",",":"),ensure_ascii=False).encode()
    if len(encoded)>IPC_MAX_RESPONSE_BYTES:
        encoded=json.dumps({"version":IPC_PROTOCOL_VERSION,"request_id":request_id,"ok":False,"result":None,"error":{"code":"IPC_RESPONSE_TOO_LARGE","field":"response","reason":"bounded response required"}},sort_keys=True,separators=(",",":")).encode()
    channel.sendall(struct.pack("!I",len(encoded))+encoded);channel.close();return 0

def _main(argv:list[str])->int:
    if len(argv)!=4 or argv[0]!="--serve-fd" or argv[2]!="--auth" or re.fullmatch(r"[0-9a-f]{64}",argv[3]) is None:return 2
    try:fd=int(argv[1])
    except ValueError:return 2
    return _serve_ipc_once(fd,argv[3])
