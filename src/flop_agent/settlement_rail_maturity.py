...
RAIL_FIELDS=frozenset({"rail_id","rail_type","implementation_version","source_class","source_id","source_sha256","code_sha256","source_status","audit_state","deployment_state","network_sha256","asset_class"})
OBS_FIELDS=frozenset({"rail_id","rail_identity_sha256","network_sha256","deployment_sha256","transaction_sha256","lock_state","terminal_state","locked_max","actual_settlement","timestamp_sha256","finality_state","timelock_policy_verified","safety_margin_verified","cross_chain_margin_verified","contract_identity_verified","source_provenance_verified"})
AUTH_FIELDS=RAIL_FIELDS|frozenset({"rail_identity_sha256"})
...
def _assess(rails_raw,observations_raw,authorities_raw,baseline_raw):
 ...
 if any(not isinstance(a,dict) or set(a)!=AUTH_FIELDS or not _digest(a["rail_identity_sha256"]) or not _valid_rail({k:a[k] for k in RAIL_FIELDS}) for a in manifest["authorities"]):return _stop(r,"ARTIFACT_SCHEMA_INVALID")
 verified=[]
 for rail in rails:
 ...
 if obs:
  ...
  for item in obs:
   matches=[(rail,a) for rail,a in verified if item["rail_id"]==rail["rail_id"] and item["rail_identity_sha256"]==a["rail_identity_sha256"]]
   if len(matches)!=1:return _stop(r,"OBSERVATION_BINDING_MISMATCH")
   rail,_=matches[0]
   if item["network_sha256"]!=rail["network_sha256"] or not item["source_provenance_verified"] or not item["contract_identity_verified"]:return _stop(r,"OBSERVATION_UNVERIFIED")
  ...
  r["rail_observation"]="EVIDENCE_PRESENT_UNVERIFIED";r["ready_for_human_review"]=True
 return _seal(r)
