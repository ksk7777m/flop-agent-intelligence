...
import hashlib, json, os, re, secrets, sqlite3, threading, weakref
...
def _build_service_family(trusted_root:Path,store_kind:str,clock:Callable[[],datetime],store_id:str)->SimpleNamespace:
    sha,jsonm,sql,osm,stm,closer=hashlib.sha256,json,sqlite3,os,stat_module,closing
    safe,hex64=SAFE_TEXT.fullmatch,HEX64.fullmatch
    ...
    def material(a:CanonicalAction)->bytes:
        if type(a) is not action_t: raise error_t("ACTION_INVALID","action","canonical action required")
    ...
    def secure_file()->None:
        ...
        try:
            ...
            try:
                ...
                finally: osm.close(fd)
            finally: osm.close(cfd)
        finally: osm.close(rfd)
    def connect()->sqlite3.Connection:
        secure_file(); con=sql.connect(str(db),timeout=10,isolation_level=None); con.execute("PRAGMA foreign_keys=ON")
        con.executescript("""CREATE TABLE IF NOT EXISTS store_metadata(singleton INTEGER PRIMARY KEY CHECK(singleton=1),store_id TEXT NOT NULL,store_kind TEXT NOT NULL,schema_version TEXT NOT NULL,policy_version TEXT NOT NULL);CREATE TABLE IF NOT EXISTS replay_records(replay_id TEXT PRIMARY KEY,nonce_scope TEXT NOT NULL UNIQUE,actor_did TEXT NOT NULL,action_class TEXT NOT NULL,context TEXT NOT NULL,nonce TEXT NOT NULL,payload_hash TEXT NOT NULL,signing_hash TEXT NOT NULL,target TEXT NOT NULL,schema_version TEXT NOT NULL,state TEXT NOT NULL,observation_count INTEGER NOT NULL,created_at TEXT NOT NULL,updated_at TEXT NOT NULL);CREATE TABLE IF NOT EXISTS replay_events(event_id INTEGER PRIMARY KEY AUTOINCREMENT,replay_id TEXT NOT NULL,from_state TEXT,to_state TEXT NOT NULL,occurred_at TEXT NOT NULL,event_hash TEXT NOT NULL UNIQUE,FOREIGN KEY(replay_id) REFERENCES replay_records(replay_id));CREATE TABLE IF NOT EXISTS effects(reservation_id TEXT PRIMARY KEY,replay_id TEXT NOT NULL,effect_identity TEXT NOT NULL,authority_ref_hash TEXT NOT NULL,effect_class TEXT NOT NULL,target TEXT NOT NULL,request_hash TEXT NOT NULL,attempt_number TEXT NOT NULL,started_at TEXT,result_state TEXT NOT NULL,confirmation_evidence_type TEXT,confirmation_evidence_hash TEXT,related_evidence_identity TEXT,verifier_revision TEXT,UNIQUE(replay_id,attempt_number),FOREIGN KEY(replay_id) REFERENCES replay_records(replay_id));""")
        con.execute("BEGIN IMMEDIATE"); row=con.execute("SELECT store_id,store_kind,schema_version,policy_version FROM store_metadata WHERE singleton=1").fetchone(); expected=(store_id,store_kind,schema,policy)
        if row is None: con.execute("INSERT INTO store_metadata VALUES(1,?,?,?,?)",expected)
        elif tuple(row)!=expected: con.rollback(); con.close(); raise error_t("STORE_AUTHORITY_MISMATCH","storage","store provenance does not match service family")
    ...
    def observe(a:CanonicalAction)->Mapping[str,Any]:
        ...
        with mutex,closer(connect()) as con:
            ...
            if prior and prior[0]!=rid: transition(con,prior[0],state_t.REPLAY_IDENTITY_CONFLICT,when); con.commit(); return proxy({"status":"DESCRIPTIVE_ONLY","decision":"REPLAY_IDENTITY_CONFLICT","replay_id":rid,"state":state_t.REPLAY_IDENTITY_CONFLICT.value,"retry_classification":retry_t.HUMAN_REVIEW_REQUIRED.value})
    ...
    def validate(a:CanonicalAction,proof:ValidationAuthority)->Mapping[str,Any]:
        ...
        with mutex,closer(connect()) as con:con.execute("BEGIN IMMEDIATE");transition(con,rid,state_t.VALIDATED,when);con.commit()
        return inspect(a)
    ...
    def attempted(reservation:Reservation)->Mapping[str,Any]:
        ...
        when=now()
        with mutex,closer(connect()) as con:con.execute("BEGIN IMMEDIATE");transition(con,binding.replay_id,state_t.EFFECT_ATTEMPTED,when);con.execute("UPDATE effects SET started_at=?,result_state=? WHERE reservation_id=?",(when,state_t.EFFECT_ATTEMPTED.value,binding.reservation_id));con.commit()
...
def _new_test_family(root:Path,clock:Callable[[],datetime],_forbidden:Path=_PRODUCTION_ROOT,
                     _builder:Callable[...,SimpleNamespace]=_build_service_family)->SimpleNamespace:
    selected=Path(root).absolute()
    if selected==_forbidden or _forbidden in selected.parents:raise ReplaySafetyError("PRODUCTION_STORE_FORBIDDEN","storage","test family cannot attach to production root")
    return _builder(selected,"TEST",clock,secrets.token_hex(32))
